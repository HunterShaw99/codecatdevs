...
export default function MapPage() {
    ...
    type BaseLayerData<T extends Layer> = {
        name: string;
        type: string;
    ...
    const [layerManager, setLayerManager] = useState<BaseLayerData<Layer>[]>([{
        name: 'default',
        type: 'scatterplot',
        colors: {fill: [255, 0, 0]},
    ...
    const addNewLayer = (newLayer: any) => {
        setLayerManager(prevLayers => [...prevLayers, {...newLayer, visible: true}]);
    };
        ...
        <div className={'max-w-full max-h-full'}>
                ...
                {isClicked &&
                    <div className="mt-2 p-2 bg-white border rounded-lg text-sm text-gray-600">
                        ...
                        <select
                            ...
                            className="p-1 rounded-lg border border-zinc-500 max-w-[200px]"
                        >
                            {layerManager.map(layer => (
            ...
            <DeckGL
                ...
                layers={[new ScatterplotLayer({
                    ...
                    ...layerManager.filter(layer => layer.visible).map(layer => {
                        if (layer.type === 'scatterplot') {
                            console.log('scatter layer', layer);
                            return new ScatterplotLayer({
                                id: `scatterplot-${layer.name}`,
                                data: layer.data,
                                getPosition: (d: any) => [d.longitude, d.latitude],
                                getRadius: 100,
                                getFillColor: layer.colors?.fill ?? [255, 0, 0],
                                radiusMinPixels: 5,
                                radiusMaxPixels: 10,
                                updateTriggers: {
