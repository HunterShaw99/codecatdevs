...
import { SLIDE_MAPPING } from '@/app/constants';
...
export default function HomePage() {
    ...
    const slides = () => {
        const posts = [];
        {
            for (const [key, value] of Object.entries(SLIDE_MAPPING)) {
                posts.push(
                <div>
                    <video className='h-auto w-150' autoPlay loop muted playsInline>
                        <source src={key} type="video/mp4" />
                        Your browser does not support the video tag.
                    </video>
                    <span className='text-center z-1 text-xs text-subtext1'>
                        {value}
                    </span>
                </div>);
            }
